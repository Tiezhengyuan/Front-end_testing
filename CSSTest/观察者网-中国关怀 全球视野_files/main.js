 					...
 					if(data.code==0){
 						for(var i=0;i<data.items.length;i++){
 							if(data.items[i].id!=null){
	 							...
	 							html += '<div class="module-member-column"><a href="https://member.'+bbs_site+'/column/'+data.items[i].column_id.toLowerCase()+'" target="_blank"><img width="28px" src="'+data.items[i].icon+'"></a><span><a href="https://member.'+bbs_site+'/column/'+data.items[i].column_id.toLowerCase()+'" target="_blank">'+data.items[i].column+'</a></span>'
 								html += '</div><div class="module-member-post" data-id="'+data.items[i].id+'">';
								html += '<h3>'+data.items[i].title+'</h3>';
	 							html += '<img src="'+data.items[i].h_pic+'">'
	 							html += '</div></div>';
 							}
 						}
 						if(html!='') html = '<div style="padding:0 10px;"><div class="module-fengwen-header"><a style="float:none" href="http://member.'+bbs_site+'" target="_blank"><span>&nbsp&nbsp&nbsp观察员</span></a><a href="http://member.'+bbs_site+'" target="_blank">查看全部>></a></div>'+html+'</div>';
 					...
 					if(data.code==0){
			 	 		...
			 	 		for(var i=0;i<max;i++){
			 	 			...
			 	 			if(data.items[i].PREVIEW_M !=''){
			 	 				html+='<img src="'+data.items[i].PREVIEW_M+'?imageMogr2/crop/!185x134">'
			 	 			}else{
