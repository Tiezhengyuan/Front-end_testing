...
(function() {
     ...
     window.mylib = {
         ...
         * doTip 确认按钮
         */
        myTip : function(obj, content, callback){
           	...
           	if(res.code == 0){
           		...
           		if(len > 0){
           			for (var i=0; i<= len - 1; i++) {
           				list.push({"name" : data[i]['name'], "url" : data[i]['url']});
           			}
           		}else{
           			return false;
           		}
           		...
           		function setHotPhrase(){
           			...
           			var url = list[count]['url'];
           			$('.hot-phrase').find('span').html('<a href="'+ url +'" target="_blank">' + name + '</a>');
           			count ++;
           		}
           		setHotPhrase();
                    ...
                    $.ajax({
                        ...
                        success: function(res) {
                            if (res.code == 0) {
                                ...
                                isSend = false;
                                mylib.msg(res.msg, 2, true);
                            } else {
                                isSend = false;
                                mylib.msg('发送失败', 2, true);
                            }
                        }
                    });
